...
def compute_window_features(window_df, channel_groups):
    ...
    for ch in all_channels:
        ...
        if envelope_col in window_df.columns:
            ...
            feats[f"{base}_env_mean"] = float(np.mean(env))
            feats[f"{base}_env_max"] = float(np.max(env))
            feats[f"{base}_env_std"] = float(np.std(env))
            envelope_means[base] = float(np.mean(env))

        if zscore_col in window_df.columns:
            z = window_df[zscore_col].values
            feats[f"{base}_z_mean"] = float(np.mean(z))
            feats[f"{base}_z_max"] = float(np.max(z))

    # Features cruzadas entre grupos musculares  
    # Capturan el patron de reclutamiento diferencial entre grupos para distinguir emociones
    group_names = list(channel_groups.keys())
    group_means = {}
    for gname, chs in channel_groups.items():
        bases = [ch.replace("_Envelope_RMS", "") for ch in chs]
        vals = [envelope_means[b] for b in bases if b in envelope_means]
        if vals:
