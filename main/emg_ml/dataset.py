...
def _windows_for_segment(seg_df, cfg: DatasetConfig, channel_groups, fs, subject_id, label, block, emotion, confidence):
    ...
    while start + win_samples <= len(seg_df):
        ...
        feats = compute_window_features(window, channel_groups)
...
def build_windows_from_labeled_csv(csv_path, subject_id, cfg: DatasetConfig = None, channel_groups=None):
    # Lee un processed_labeled.csv y devuelve un DataFrame de features, una fila por ventana
    if cfg is None:
        cfg = DatasetConfig()
    groups = channel_groups if channel_groups is not None else cfg.channel_groups
    ...
    for _, seg_df in df.groupby(segment_id):
        ...
        rows = _windows_for_segment(seg_df, cfg, groups, fs, subject_id, label, block, emotion, confidence)
        all_rows.extend(rows)
 
    return pd.DataFrame(all_rows)


def build_dataset(subject_specs, cfg: DatasetConfig = None, output_path=None):
    """
    subject_specs: dict donde cada valor puede ser:
      - un string con la ruta al processed_labeled.csv, o
      - una tupla cuando el agrupamiento de canales varia entre sujetos
 
    Ejemplo:
        subject_specs = {
            "Test1": ("Test1/Data/processed_labeled.csv",
                      {"Grupo_A": ["EMG1_Envelope_RMS","EMG4_Envelope_RMS"],
                       "Grupo_B": ["EMG2_Envelope_RMS","EMG3_Envelope_RMS"]}),
        }
 
    Devuelve el DataFrame combinado y opcionalmente lo guarda en output_path.
    """
    if cfg is None:
        cfg = DatasetConfig()
    ...
    for subject_id, spec in subject_specs.items():
        if isinstance(spec, (tuple, list)):
            csv_path, groups = spec
        else:
            csv_path, groups = spec, cfg.channel_groups
 
        print(f"  Procesando {subject_id}: {csv_path}")
        print(f"    channel_groups: {groups}")
        wdf = build_windows_from_labeled_csv(csv_path, subject_id, cfg, channel_groups=groups)
